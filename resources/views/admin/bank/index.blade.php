...
@extends('layouts.app', ['title' => _lang('bank/cash'), 'modal' => 'full'])
@section('page.header')
	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active">{{ _lang('bank/cash') }}</span>
			</div>
...
<div class="card border-top-success rounded-top-0" id="table_card">
	<div class="card-header header-elements-inline bg-light border-grey-300" >
		<h5 class="card-title">{{ _lang('bank/cash') }}
		<button type="button" class="btn btn-outline alpha-info text-info-800 border-info-600 rounded-round" id="content_managment" data-url="{{route($route.'create')}}"><i class="icon-stack-plus mr-1"></i> {{ _lang('add_new_bank/cash') }}</button>
		</h5>
		...
		<div id="table_display">
			<table class="table content_managment_table" data-url="{{route($route.'datatable')}}">
				<thead>
					<tr>
						<th>{{ _lang('id') }}</th>
						<th>{{ _lang('account_name') }}</th>
						<th>{{ _lang('oppening_balance') }}</th>
						<th>{{ _lang('note') }}</th>
						<th>{{ _lang('action') }}</th>
					</tr>
				</thead>
